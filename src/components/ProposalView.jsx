...
const ProposalView = () => {
  ...
  const moveRejectButton = () => {
    if (!containerRef.current || !rejectButtonRef.current) return
    ...
    const maxX = container.width - buttonWidth - 60
    const maxY = container.height - buttonHeight - 60
    const minX = 20
    const minY = 20
    
    // Generate random position in different quadrants to make it more unpredictable
    const quadrants = [
      { x: minX + Math.random() * (maxX * 0.4), y: minY + Math.random() * (maxY * 0.4) }, // top-left
      { x: maxX * 0.6 + Math.random() * (maxX * 0.4), y: minY + Math.random() * (maxY * 0.4) }, // top-right
      { x: minX + Math.random() * (maxX * 0.4), y: maxY * 0.6 + Math.random() * (maxY * 0.4) }, // bottom-left
      { x: maxX * 0.6 + Math.random() * (maxX * 0.4), y: maxY * 0.6 + Math.random() * (maxY * 0.4) }, // bottom-right
    ]
    
    const randomQuadrant = quadrants[Math.floor(Math.random() * quadrants.length)]
    setRejectButtonPosition(randomQuadrant)
            ...
            <div className="proposal-content">
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={rejectAttempts < 5 ? {} : { scale: 1.05 }}
                  style={{
                    position: rejectAttempts > 0 ? 'absolute' : 'static',
                    cursor: rejectAttempts < 5 ? 'pointer' : 'pointer'
                  }}
                >
                  <X size={20} />
                  {rejectAttempts >= 5 ? 'Fine, No!' : 'No'}
                </motion.button>
              </motion.div>
